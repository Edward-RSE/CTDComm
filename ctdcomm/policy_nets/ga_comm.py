...
class GACommNetMLP(nn.Module):
    def __init__(self, args, num_inputs):
        ...
        if args.cave:
            ...
            value_input_size = 2*self.hid_size + (args.nagents * (args.nagents - 1))
    ...
    def forward(self, x, info={}):
        ...
        if not self.args.comm_mask_zero:
            ...
            if self.args.save_adjacency:
                adjacency_data = torch.zeros([n, n])
        ...
        if self.cave:
            ...
            stacked_adjacency = torch.stack([torch.flatten(adjacency_data) for _ in range(batch_size*n)])
            # print("Stacked adjacency", stacked_adjacency.shape)
            # print("hidden_state", hidden_state.shape)
            # print("comm", comm.view(batch_size*n, self.hid_size).shape)
            value_input = torch.cat((hidden_state, comm.view(batch_size*n, self.hid_size), stacked_adjacency), dim=-1)
            # print("value_input", value_input.shape)
            # assert False
            # value_input = torch.cat([value_input, stacked_adjacency], dim=-1)
        ...
        if self.args.save_adjacency and self.args.recurrent:
            return action, value_head, (hidden_state.clone(), cell_state.clone()), [comm_density1, comm_density2], adjacency_data.detach().numpy()
        elif self.args.save_adjacency:
            return action, value_head, [comm_density1, comm_density2], adjacency_data
