...
from ctdcomm.policy_nets.gnn_layers import GraphAttention
...
class MAGIC(nn.Module):
    ...
    def __init__(self, args, num_inputs):
        ...
        self.obs_encoder = nn.Linear(num_inputs, args.hid_size) #previously args.obs_size, should be the same
    ...
    def forward(self, x, info={}):
        ...
        if self.args.save_adjacency:
            # Always 2 communication passes in MAGIC
            adjacency_data = torch.zeros([2, n, n])
        ...
        if self.args.save_adjacency:
            return action_out, value_head, (hidden_state.clone(), cell_state.clone()), adjacency_data.detach().numpy()
        else:
            return action_out, value_head, (hidden_state.clone(), cell_state.clone())
