...
def translate_local_obs(obs):
    ...
    if len_obs > local_total_dim:
        print(
            "tlocal_total_dim (%s) too small, obs dim is %s."
            % (local_total_dim, len(obs))
        )
    ...
    if enable_task_embedding:
        new_obs = new_obs.reshape(
            (*ori_shape[:-1], local_total_dim + task_embedding_dim)
        )
    else:
...
def single_translate_local(ori_obs, map_info):
    ...
    # dead agents
    if np.all((ori_obs[: len(ori_obs) - ori_num_agents] == 0.0)):
    ...
    new_ally_feats = np.zeros(
        (target_num_agent - 1, local_ally_feats_dim), dtype=np.float32
    )
    new_enemy_feats = np.zeros(
        (target_num_enemy, local_enemy_feats_dim), dtype=np.float32
    )
    ...
    valid_idx = np.argwhere(np.any((ori_ally_feats != 0.0), axis=-1) > 0).reshape(-1)
    ...
    if ori_unit_type_bits > 0:
        ...
        ori_types = np.argwhere(ori_type_bits == 1.0)[:, -1]
    else:
        ori_types = np.zeros(len(valid_idx), dtype=np.int)
    unified_types = agent_types[ori_types]
    new_ally_feats[valid_idx, unified_types + 6] = 1.0
    idx += ori_unit_type_bits

    new_ally_feats[valid_idx, 16 : 16 + ori_action_dim] = ori_ally_feats[
        valid_idx, idx:
    ]

    # encode enemy features
    valid_idx = np.argwhere(np.any((ori_enemy_feats != 0.0), axis=-1) > 0).reshape(-1)
    ...
    new_enemy_feats[valid_idx, unified_types + 6] = 1.0
    ...
    if ori_unit_type_bits > 0:
        ...
        ori_types = np.argwhere(ori_type_bits == 1.0)[0][0]
    else:
        ori_types = 0
    unified_types = agent_types[ori_types]
    new_own_feats[unified_types + 6] = 1.0
    idx += ori_unit_type_bits
...
def gen_task_embedding(map_name):
    ...
    task_embedding[2 + race_map[map_info["a_race"]]] = 1.0
    task_embedding[5 + race_map[map_info["b_race"]]] = 1.0
